...
const SingleItem = ({ item }: { item: any }) => {
  ...
  const handleRemoveFromWishlist = async () => {
    try {
      if (accessToken) {
        const response = await fetch(API_ENDPOINTS.REMOVE_FROM_WISHLIST(item.id), {
          method: "DELETE",
          headers: {
  ...
  return (
    <div className="flex items-center border-t border-gray-3 py-5 px-10 hover:bg-gray-50 transition-colors">
      <div className="min-w-[400px]">
        <div className="flex items-center gap-5">
          <Link 
            href={`/shop-details/${item.id}`}
            className="flex-shrink-0 flex items-center justify-center rounded-lg bg-gray-2 w-20 h-20 overflow-hidden hover:opacity-80 transition-opacity"
          >
      ...
      <div className="min-w-[265px] text-center">
        {item.status === "In Stock" ? (
          <span className="text-base  text-green">In Stock</span>
        ) : (
          <span className="text-base  text-red">Out of Stock</span>
        )}
      </div>

      <div className="min-w-[205px] text-center">
        <p className="text-dark text-lg">₹{item.discountedPrice}</p>
        {item.price > item.discountedPrice && (
          <p className="text-gray-400 line-through text-sm font-normal">₹{item.price}</p>
        )}
      </div>
